  ...
  urls.forEach((url) => {
      ...
      script.addEventListener('load', () => {
        ...
        if (!urls.some(u => !u.done)) {
            ...
            function gtfo() {
                window.$( "span:contains('Suggested for you')" ).parent().parent().parent().parent().parent().parent().parent().parent().css('visibility', 'hidden');
                window.$( "span:contains('Sponsored')" ).parent().parent().parent().parent().parent().parent().parent().parent().css('visibility', 'hidden');
            }
...
//
// (C) 2009 - 2023 Stephen Fernandez - https://www.fbpurity.com
//
...
// 36.1.0   1stJan2022  Fixed FBP not processing occasional posts in the Newsfeed, Fixed duplicate "back to top posts" link bug, fixed position of "X" close button on photos
// 36.2.0   5thJan2022  Fixed FBP news update checker, added link to "show deactivated friends" function to top of https://www.facebook.com/friends  page
// 36.3.0   5thJan2022  Added support for the "GM." API (GM.setValue + GM.getValue) to fix compatibility with the "UserScripts" extension for Safari, beefed up the hide notifications popup option
// 36.4.4  30thJan2022  Added "name change" alerts, for when your fb friends change their names, fixed autohide filter, fixes hide whole left column and hide whole right column options, fixed autoclick "see more" in left column, fixed hide offline friends in chat option, fixes hiding right hand column sections
// 36.4.5   1stFeb2022  Fixed deleted friend alerts were showing even if the deleted friend alerts option was turned off.
// 35.4.7   2ndFeb2022  Fixed bug in hide whole left column (it was hiding the whole newsfeed for some people)
// 36.4.8  14thFeb2022  Automatically hide "disable ad blocker" message
// 36.4.10 17thFeh2022  Fixed hiding meta quest and ray ban stories left column link options, fixed the clear "ad topics" functionality, fixed hide whole right column option (image comments no longer hidden)
// 36.4.12 23rdFeb2022  Fixed word for "Sponsored" in german
// 36.4.14 29thMar2022  Fixed the "Clear ad topics" button, fixed hide "ray bans stories" and hide "welcome" left column link options
// 36.5.0   3rdApr2022  Added a hide "Comment Suggestions" option, fixed hide stories link in left column if hide stories option is enabled, fixed the hide "News" left column link option, fixed the clear "other categories used to reach you" button/functionality on the ad preferences page, fixed the hide "ad topics" button on the facebook ad settings page

// (C) Stephen Fernandez 2009-2023   https://www.fbpurity.com
...
(function() {
  ...
  var FBPfanpagesmallproficon="https://i.imgur.com/lHHVfYU.png";//https://i.imgur.com/m3ezkAp.png
...
['Campus','lccampus',''],
['Climate Science Information Center','lcclimatescience',''],
...
['Facebook Pay','lcpayments','Hide Facebook Pay and all links under the Payments Heading (including Send or Request Money)'],
...
['Pokes','lcpokes',''],
['Ray Ban Stories','lcraybans','Hide the Ray Ban Stories link in the left column'],
['Reels','lcreels',''],
...
'<table><tr><td><a target="_blank" href="https://donate.fbpurity.com"><img style="margin-top:3px" border="0" height="47" width="160" alt="Donate to FBP" src="https://www.paypalobjects.com/en_US/GB/i/btn/btn_donateCC_LG.gif"></a></td><td>&nbsp;</td><td>Please tell all your friends about FBP too</td><td><a href="https://www.facebook.com/sharer.php?u=https://www.facebook.com/' + FBPFanPageURL + '#fbpshr" title="Share on Facebook" target="_blank"><img style="vertical-align:bottom" width="123" height="22" border="0" src="https://sites.google.com/site/st333v/fbp-1/shareonfacebook.gif" alt="Share on Facebook"></a></td></tr></table>'+
'</div>'+
'</td></tr>'+
...
'<tr title="Hide the Stories box and also the &quot;Your Story&quot; section below the Status update box" style="background-color:#ECEFF5 !important"><td><label class="fbpoptslabel" for="rcstories">Stories</label></td><td><input type="checkbox" id="rcstories"></td></tr>'+
...
'<p></p><p><b>Next Step:</b> <a id="clickforinstructions" onclick="document.getElementById(\'firstpara\').style.display=\'none\';document.getElementById(\'secondpara\').style.display=\'block\';" href="javascript:;">Click here for instructions on how to use F.B. Purity</a></p></div>'+
...
function fbploadvalue(name,funct){
  ...
  try {
    if(isgmapi==1) {
	  ...
	  GM.getValue(name,"").then(function(value){console.log("new method val=", value);funct(value)}); //
      val=-999;
      //return val;
	}
	else if(ischrome) {
...
function fbpfirstruncheck(result) {
  ...
  if (((fbpfirstrun=='') || (typeof(fbpfirstrun)=='undefined') || (fbpfirstrun==null)) && (!(window.location.href.match(/^https?:\/\/apps\.facebook|^https?:\/\/blog\.facebook|^https?:\/\/secure\.facebook/)))) {
    if(!document.getElementById('fbpfirstrundiv')) {
	  ...
	  if(ph.firstChild)
  	    window.setTimeout(function(){ph.firstChild.parentNode.insertBefore(firstrundiv, ph.firstChild.nextSibling);if(document.querySelectorAll("#ssrb_root_start,#fbpfirstrundiv").length==2){
		  ...
		  document.getElementById('clickforinstructions').addEventListener('click',function(){document.getElementById('firstpara').style.display='none';document.getElementById('secondpara').style.display='block'}, false);
	    }},3000);
...
var fixsortorder = function() {
  ...
  if((fbpoptsobj.sortmostrecent) && !window.location.hash.match(/#topstories/)) { // force feed to sort by most recent //  || fbpoptsobj.sortfriendfeed
	...
	*/
    ...
    function fbpRemoveEventListeners ( nodetofix ) { // this funtion is causing the blank screen issue... but why?
	  //nodetofix.style.border="5px solid red";  // commented out this functions contents because it was causing the blank screen bug
      //var new_element = nodetofix.cloneNode(true);
      //nodetofix.parentNode.replaceChild(new_element, nodetofix);
      //new_element=null;
    }
...
var initstage2 = function () {
  ...
  */
  ...
  if(fbpoptsobj.commentwall==1) {
	headerTextFilter += ",posted in,commented on .* post,commented on .* article,wrote on .*\'s timeline, to .*\'s timeline, shared a (post|link|note) to the group"; //commented on h.. own post,commented on .*\'s post in,commented on a Post,
...
// hide sponsored posts in newsfeed
  if(fbpoptsobj.sponsoredbox==1) {
    ...
    extraTextFilter += ",Recently Released Albums,Get Offer ·,Pages You May Like,Links you may like,Page Stories you may like,Jobs Near You,^Recommended for you,^Suggested because it's popular on Facebook,X?Events You May Like,Advertiser-sponsored poll,suggested shows,Get (every )?New episode(s)? in your watchlist,· Paid ·,· Paid for by,Paid partnership ·,Paid Partnership  ·,^Similar to posts";
...
// top nav background image
  ...
  if(fbpoptsobj.newsfeed) {
	...
	fbpfreestyle.textContent +=' #ssrb_feed_start {display:block !important} #ssrb_feed_start:after {content: "Your Newsfeed is hidden because you have selected FBPs \'Hide The Whole Newsfeed\' option"}';
	fbpfreestyle.textContent +=' #ssrb_feed_start+div[role="feed"], #ssrb_feed_start + div {display:none}';
	fbpfreestyle.textContent +=' #fbpurityinfowrapper+h3+div {display:none} #fbpurityinfowrapper:after {content: "Your Newsfeed is hidden because you have selected FBPs \'Hide The Whole Newsfeed\' option"}';
  }
  // hide home link
  ...
  for(var b=0;b<lcoptsarr.length;b++)
	...
	if(fbpoptsobj.lcnotes==1)
      ...
      fbpfreestyle.textContent += ' ' + leftrailcode3 + ' a[href*="www.oculus.com"], div[class="__fb-light-mode"] a[href*="www.oculus.com"], a[href*="www.oculus.com"] {display:none !important}'; // can probably delete the first 2 rules and just leave the last one
    ...
    if(fbpoptsobj.lcreels==1)
      fbpfreestyle.textContent +=' ' + leftrailcode3 + ' a[href*="/reel/"],li>div>a[href*="/reel/"] {display:none !important}';
    ...
    if(fbpoptsobj.lcraybans==1)
      fbpfreestyle.textContent += ' ' + leftrailcode3 + ' a[href*="www.ray-ban.com"], div[data-isanimatedlayout] a[href*="www.ray-ban.com"], li>div>a[href*="www.ray-ban.com"], a[href*="www.meta.com%2Fglasses"], a[href^="https://www.meta.com/glasses"] {display:none !important}';
	...
	if(fbpoptsobj.lcwatch==1)
      fbpfreestyle.textContent +=' ' + leftrailcode3 + ' a[href*="/watch/"],li>div>a[href$="https://www.facebook.com/watch/"], ' + leftrailcode2 + ' a[href*="/watch/"], a[href="https://www.facebook.com/watch/?ref=mega_menu"], div[role="banner"]+div[data-isanimatedlayout] a[href*="/watch/"] {display:none !important}';
    ...
    */
    ...
    if(fbpoptsobj.rcstories==1) {
	  ...
	  extraTextFilter+= ',See full-?screen photos and videos before they disappear';
    }
...
function buildTextFilterRX(filterName,additionalFilterText){
  ...
  if(filterName=="AutoHide"){
    ...
    if(fbpoptsobj.reelsvideo){ // autohide reels
      if (tmptextfilter.length)
	    tmptextfilter+=',^Reels,· Reels  ·,Reels and short videos';
      else
	    tmptextfilter+='^Reels,· Reels  ·,Reels and short videos';
    }
  }
...
if(photosubjectfilterlist.length) {
  ...
  if( fbpoptsobj.hidemesstopnav) {
    fbpboxstyles +='#pagelet_bluebar a[name="mercurymessages"], div[role="banner"] div[aria-label="Messenger"]{display:none}';
  }
  ...
  if((typeof fbpoptsobj.hideanimatedgifs !=='undefined') && fbpoptsobj.hideanimatedgifs) {
	fbpboxstyles += ' div[id^="gif_comment_attachment"], div[data-testid^="UFI2Comment"] img[src*="ext=gif"],form.commentable_item img[src*="ext=gif"], div[data-testid^="UFI2Comment"] video._ox1, form.commentable_item video._ox1, div[aria-label^="Comment by"] img[src*=".gif"], div[aria-label^="Comment by"] video, video[aria-label^="GIF"] {display: none}';
  }
...
// END ad hider for new facebook design
  ...
  function cleartheshizzle(thenode) {
	...
	if(!(new2021style || new2022style || newest2022style) ) {
	  if(debug)
	...
	*/
	...
	var svguse,svgusexlink,shadowroot;
	  ...
	  */
	  ...
	  try{
	    ...
	    if(svguse){
		  ...
		  if(svgusexlink) {
		    svgusexlinktext=document.querySelector(svgusexlink).textContent;
	  ...
	  if(fbpoptsobj.imagetagsonhover) { // show image classification text when mouse pointer hovers over single image post
		...
		if(imageisclassified.length) {
		  for(var imgindex=0; imgindex<imageisclassified.length; imgindex++) {
		    if(imageisclassified[imgindex].getAttribute('alt') && imageisclassified[imgindex].getAttribute('alt').length){
        ...
        if(blockit) {
		  ...
		  if (fbpoptsobj.postedlink) { //&& (storynodes[i].getAttribute('class') && (!storynodes[i].getAttribute('class').match(/fbp//)))
		    ...
		    if(storynodes[i].querySelector('a[rel^="nofollow noopener"]')) {
			  ...
			  if(fbpoptsobj.sponsoredbox)
			    createObserver(storynodes[i]);
			  /* commenting out 5/may/2020 to test if ad blocking still works and memory issues improve
			  if (fbpoptsobj.sponsoredbox)
			    createObserver(storynodes[i]);//check for sneaky ad insertions
			  */
			}
			...
			//setTimeout(adcatcher,4000,storynodes[i],adString,2);

			/*if((unravelenigmad(storynodes[i])).match(adString.substring(1))){
			  if(debug)
			    storynodes[i].style.border="3px red solid";
			  else{
				storynodes[i].classList.remove("fbpnormal", "fbpblocked", "fbpblockedx");
	            storynodes[i].classList.add("fbad");
	            storynodes[i].style="display:none !important";
			  }
			  console.log('found enigma ad');
...
// BEGIN inject deleterecentactivitymodule code into fb page
  ...
  // try new injection method thats compatible with MV3
...
function processpagesobject(pagesobj){
...
function hidegroupconvos() {
  ...
  if(document.querySelectorAll('div[role="complementary"] h3').length ){
    for(var i=0;i<document.querySelectorAll('div[role="complementary"] h3').length;i++)
      if(document.querySelectorAll('div[role="complementary"] h3')[i].textContent.match(/^Group Conversations/i)){
...
function addfbpfaqprompt() {
  ...
  if(document.getElementById('photos_snowlift') && (!document.getElementById('photos_snowlift').className.match(/hidden_elem/)) ) {
    ...
    //console.log('yeehaw'); // this "if" section not currently working and maybe redundant...
	if(!document.querySelector('div.fbpfaqprompt')) {
      imputs=document.querySelector('form[role="presentation"]');
	  imputs.parentNode.parentNode.appendChild(fbpfaqprompt);
	}
  }
  else if(document.querySelector('form[role="presentation"]')){ //  new fb design
	//console.log('hooohooo');
	imputs=document.querySelectorAll('form[role="presentation"]');
	for(i=0;i<imputs.length;i++) {
	  if(!imputs[i].parentNode.parentNode.querySelector('div.fbpfaqprompt'))
		//imputs[i].parentNode.insertBefore(fbpfaqprompt.cloneNode(true),imputs[i]); // fixed 16march2022
	    imputs[i].parentNode.parentNode.insertBefore(fbpfaqprompt.cloneNode(true),imputs[i].parentNode);
    }
  }
  else {
	if(debug)
...
function checkdelfriendstorage(delfriendsjson) {
  ...
  if(!(document.getElementById('ssrb_left_rail_start') || document.querySelector('div[data-isanimatedlayout] div[data-isanimatedlayout]')) || (!(fbpoptsobj.deletedfriendalerts || fbpoptsobj.namechangealerts)))
    return;
  ...
  if(!delfriendsjson || delfriendsjson=="") {
	...
	if( (document.querySelector('#ssrb_left_rail_start') || document.querySelector('div[data-isanimatedlayout] div[data-isanimatedlayout]')) && !(document.getElementById('replaydeletedfriendsalert'))) {
	  ...
	  tmpli.id="leftnavdfalinkli";
	  tmpli.setAttribute('class','clearfix sideNavItem stat_elem');
      tmpli.innerHTML='<div style="background-color:pink"><a title="Replay Friends Alert" id="replaydeletedfriendsalert" class="_5afe" style="margin-left:-24px !important;display:inline-block" href="javascript:;"><img src="' + friendIconClass + '" style="vertical-align:middle"></img>&nbsp;Friends Alert &nbsp;<span style="color:#4e5665">'+ Object.keys(delfriendsobj).length + '</span></a>&nbsp;<a id="clearleftnavdfalink" style="color:red;display:inline-block" href="javascript:;" title="Clear Friends Alert">X</a></div>';
...
function clearadprefs() {
  ...
  else if (location.href.match(/\/adpreferences\/ad_topics/)) {	
...
// we should check to see if fbpurify is already running...
  ...
  var waitforbody=window.setInterval(function(){
	...
	try{
	  ...
	  if(window.location.href.match(/\/adpreferences\/?/)) {
...
function setCurrentSPCode(spCode) { //redundant code (for now) as facebook changed the way they deliver sponsored posts
  fbpoptsobj.currentSPCode=spCode;
  fbpsavevalue('fbpoptsjson-' + currentuserid, JSON.stringify(fbpoptsobj));
...
function alertadword(e){
	...
	if(confirm(spConfirmString))
	  ...
	  if(e.target.getAttribute && 
         e.target.getAttribute('fbpuid')  && 
		 document.querySelector("div[aria-labelledby='" + e.target.getAttribute('fbpuid') + "']") && 
		 document.querySelector("div[aria-labelledby='" + e.target.getAttribute('fbpuid') + "'] a[role=\"link\"]") &&
		 (document.querySelectorAll("div[aria-labelledby='" + e.target.getAttribute('fbpuid') + "'] a[role=\"link\"]").length>2)  && 
		 document.querySelectorAll("div[aria-labelledby='" + e.target.getAttribute('fbpuid') + "'] a[role=\"link\"]")[2].textContent ) { //&&  document.querySelectorAll("div[aria-labelledby='" + e.target.getAttribute('fbpuid') + "'] a[role=\"link\"]")[2].textContent.match(/·\s+([\p{L}0-9]*)\s+·/u)
		//setCurrentSPCode(document.querySelector("div[aria-labelledby='" + e.target.getAttribute('fbpuid') + "']").textContent.match(/·\s+([a-zA-Z0-9]*)\s+·/)[1]); 
	    //setCurrentSPCode(document.querySelector("div[aria-labelledby='" + e.target.getAttribute('fbpuid') + "']").textContent.match(/·\s+([\p{L}0-9]*)\s+·/u)[1]); // added support for other languages
		
		setCurrentSPCode(document.querySelectorAll("div[aria-labelledby='" + e.target.getAttribute('fbpuid') + "'] a[role='link']")[2].textContent); // .match(/·\s+([\p{L}0-9]*)\s+·/u)[1] // added support for other languages
		if(debug)
...
function handleIntersect(entries, observer) {
  entries.forEach(function(entry) {
	...
	window.setTimeout(adcatcher,100,entry.target,adString,2);
